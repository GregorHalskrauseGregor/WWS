...
const DATEIEN = ['artikelklassen', 'warengruppen', 'merkmale', 'synonyme',
  'dimensionen', 'einheiten', 'zulassungen', 'gelernt', 'umgangssprache'];
...
function promptKontext(artikelart = null) {
  ...
  // Umgangssprache: Handwerker-Floskeln und Marken, die der Bot erkennen soll.
  // Strukturiert (Tabelle) + freier Hinweistext. Beides geht in den Prompt.
  const umg = w.umgangssprache;
  if (umg) {
    const syn = umg.synonym || [];
    const hinweis = (umg.hinweis || '').trim();
    if (syn.length) {
      zeilen.push('\nHANDWERKER-UMGANGSSPRACHE (Artikel-Aliasse, vom Code und der KI nutzbar):');
      for (const s of syn) {
        const gelt = s.gilt_fuer && s.gilt_fuer.length
          ? ` [gilt für: ${s.gilt_fuer.join(', ')}]`
          : '';
        zeilen.push(`  "${s.von}" → "${s.nach}"${gelt}`);
      }
    }
    if (hinweis) {
...
// Liefert die Liste der Synonym-Eintraege (von/nach) für die Code-Anwendung.
function umgangsspracheSynonyme() {
  return (lade().umgangssprache && lade().umgangssprache.synonym) || [];
}

// Liefert den freien Hinweistext (für Tests/Logging).
function umgangsspracheHinweis() {
  const u = lade().umgangssprache;
  return (u && u.hinweis || '').trim();
}

// Ersetzt im Text alle vorkommenden Synonyme (Wort-genau, case-insensitive).
// Liefere EINE kanonisierte Form, in der jedes Synonym an der jeweils ersten
// passenden Stelle ersetzt ist. Mehrfachersetzungen sind möglich, wenn der
// Ausgangstext mehrere Synonyme enthält ("Prestabo 22mm Edelstahl" →
// "Pressfitting C-Stahl 22mm Edelstahl" nur, wenn "Pressfitting C-Stahl" selbst
// auch in der Tabelle steht).
function wendeUmgangsspracheAn(text) {
  if (!text) return text;
  let raus = text;
  for (const s of umgangsspracheSynonyme()) {
    if (!s || !s.von || !s.nach) continue;
    // Wortgrenzen, damit "Cu" nicht in "Cupferrohr" matcht und umgekehrt
    // "Kupfer" nicht in "Kupferrohr" durch "Kupferfitting" ersetzt wird.
    const regex = new RegExp(`\\b${escapeRegex(s.von)}\\b`, 'gi');
    raus = raus.replace(regex, s.nach);
  }
  return raus;
}
...
module.exports = {
  ORDNER, DATEIEN,
  lade, neuLaden,
  umgangsspracheSynonyme, umgangsspracheHinweis, wendeUmgangsspracheAn,

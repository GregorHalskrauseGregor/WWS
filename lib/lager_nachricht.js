...
// Die Rueckfrage bei einer Teilmenge. Sie ist der Grund, warum der Lagerist
// nichts entscheiden muss: er sagt nicht "5 reichen", er bestaetigt nur, dass
// mehr nicht da war. Aus dieser Tatsache zieht der Bot die Folgerung.
function restFrage(r, index, gefunden) {
  const p = r.positionen[index];
  const zeilen = [
    '⚠️ *Weniger als angefordert*',
    '',
    `*${p.bezeichnung}*`,
    `Angefordert: ${p.menge} ${p.einheit}`,
    gefunden === 0 ? 'Du meldest: *nichts da*' : `Du gibst mit: *${gefunden} ${p.einheit}*`,
    '',
    gefunden === 0
      ? 'Heißt das, von der Position ist nichts mehr im Lager?'
      : `Heißt das, das war der Rest — nach dem Herauslegen ist die Position leer?`,
    '',
    '_Wenn ja, melde ich den Bestand als leer._'
  ];
  return zeilen.join('\n');
}
...
function abschlussFrage(r) {
  ...
  for (const p of r.positionen) {
    ...
    const menge = b >= p.menge ? `${p.menge} ${p.einheit}`
      : b > 0 ? `nur ${b} von ${p.menge} ${p.einheit} — Rest war leer`
      : 'nicht da';
...
// Was der Monteur beim Zurueckziehen sieht — oder warum es nicht mehr geht.
function stornoBescheid(r, erfolg, grund) {
  if (erfolg) {
    return [
      `↩️ *Reservierung ${r.id} zurückgezogen*`,
      `_${zeitpunkt(r.erstelltAm)}_`,
      '',
      ...r.positionen.map((p) => `· ${p.menge} ${p.einheit}  ${p.bezeichnung}`),
      '',
      'Das Material ist wieder frei für alle.'
    ].join('\n');
  }
  if (grund === 'zu_spaet') {
    return [
      `🔒 *${r.id} lässt sich nicht mehr zurückziehen*`,
      '',
      'Der Lagerist hat sie bereits bearbeitet — das Material ist herausgelegt ' +
      'oder verbucht.',
      '',
      '_Du musst nehmen, was zusammengestellt wurde. Was du nicht brauchst, ' +
      'räum bitte selbst zurück und lager es wieder ein._'
    ].join('\n');
  }
  if (grund === 'fremd') return `Die Reservierung ${r.id} gehört nicht dir.`;
  return 'Diese Reservierung gibt es nicht.';
}
...
module.exports = {
  offeneListe, positionsFrage, restFrage, abschlussFrage, monteurBescheid, stornoBescheid,

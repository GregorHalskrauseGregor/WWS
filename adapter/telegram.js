...
function starte({ token, provider, antwortChat, routerChat, extraktionChat, summaryChat }) {
  // ══════════════════════════════════════════════════════════════════════
  // WELCHE UPDATES TELEGRAM UEBERHAUPT SCHICKEN SOLL
  // ══════════════════════════════════════════════════════════════════════
  //
  // Telegram MERKT SICH, welche Update-Arten zuletzt angefordert wurden — und
  // liefert bis auf Weiteres nur die. Wurde irgendwann einmal etwas Engeres
  // gesetzt, etwa nur "message", kommen Knopfdruecke (callback_query) nie mehr
  // an. Ohne Fehlermeldung, ohne Logzeile: der Knopf blinkt und versandet.
  //
  // Das ist von aussen nicht zu sehen und hat hier Stunden gekostet. Deshalb
  // wird die Liste jetzt bei JEDEM Start ausdruecklich mitgeschickt, statt
  // sich auf einen Zustand zu verlassen, den niemand nachsehen kann.
  //
  // Als JSON-Zeichenkette, nicht als Array: die Bibliothek haengt ein Array
  // sonst als allowed_updates[0]=... an, und damit kann Telegram nichts
  // anfangen.
  const bot = new TelegramBot(token, {
    polling: {
      params: {
        allowed_updates: JSON.stringify([
          'message',           // Text, Sprache, Fotos, Dateien
          'edited_message',    // nachtraeglich bearbeitete Nachrichten
          'callback_query',    // Knopfdruecke — genau das fehlte moeglicherweise
          'my_chat_member'     // Bot wurde einer Gruppe hinzugefuegt/entfernt
        ])
      }
    }
  });
